...
export class LvlService {
  ...
  async findItem(_id: number): Promise<Lvl> {
    return await this.lvlRepository
      .findOneOrFail({ where: { id: _id }, relations: { workers: true } })
      .catch(() => {
  ...
  async update(_id: number, DTO: UpdateLvlDTO): Promise<Lvl> {
    ...
    const original = await this.lvlRepository.findOne({
      where: { id: _id },
    ...
    const updated = await this.lvlRepository.findOne({
      where: { id: _id },
      relations: { workers: true },
    });
    return updated;
