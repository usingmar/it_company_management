...
export class CountryService {
  ...
  async findItem(_id: number): Promise<Country> {
    return await this.countryRepository
      .findOneOrFail({ where: { id: _id }, relations: { companies: true } })
      .catch(() => {
  ...
  async update(_id: number, DTO: UpdateCountryDTO): Promise<Country> {
    ...
    const original = await this.countryRepository.findOne({
      where: { id: _id },
      relations: { companies: true },
    });
    if (original) {
    ...
    const updated = await this.countryRepository.findOne({
      where: { id: _id },
      relations: { companies: true },
    });
    return updated;
