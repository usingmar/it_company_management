...
export class TechnologyService {
  ...
  async findItem(_id: number): Promise<Technology> {
    return await this.technologyRepository
      .findOneOrFail({ where: { id: _id }, relations: { projects: true } })
      .catch(() => {
  ...
  async update(_id: number, DTO: UpdateTechnologyDTO): Promise<Technology> {
    ...
    const original = await this.technologyRepository.findOne({
      where: { id: _id },
      relations: { projects: true },
    });
    if (original) {
      await this.technologyRepository.save({
        id: parseInt(_id.toString()),
        ...DTO,
      });
    }
    const updated = await this.technologyRepository.findOne({
      where: { id: _id },
      relations: { projects: true },
    });
    return updated;
